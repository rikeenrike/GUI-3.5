...
public class JSON {
   ...
   static JSONArray reader() throws IOException, ParseException, org.json.simple.parser.ParseException{
        FileReader reader = new FileReader("src\\z.STUDLIST.json");
        if (reader.ready()) {
            ...
            if (!line.equals("")) {
                reader.close();
                FileReader reader2 = new FileReader("src\\z.STUDLIST.json");
    ...
    static void writer() throws IOException {
        FileWriter writer = new FileWriter("src\\z.STUDLIST.json");
